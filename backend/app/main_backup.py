...
def load_trained_models():
    ...
    try:
        ...
        # Load ensemble transfer learning model
        ...
        print(f"Successfully loaded {len(trained_models)} ANFIS models and 1 transfer learning model")
        return len(trained_models) > 0 and transfer_model is not None
...
def predict_with_anfis(image_path: str) -> dict:
    ...
    if not trained_models or not preprocessing_data:
        return {
            ...
            'confidence': 0.0,
            'probabilities': {},
            'message': 'Trained models not available'
        }
    ...
    try:
        ...
        return {
            ...
            'message': f'ANFIS classified as {best_class} with {confidence:.3f} confidence'
        }
...
def predict_with_transfer_learning(image_path: str) -> dict:
    ...
    if transfer_model is None or not preprocessing_data:
        return {
            ...
            'confidence': 0.0,
            'probabilities': {},
            'message': 'Transfer learning model not available'
        }
    ...
    try:
        ...
        return {
            ...
            'message': f'Transfer Learning classified as {predicted_class} with {confidence:.3f} confidence'
        }
...
def predict_with_ensemble(image_path: str) -> dict:
    ...
    try:
        ...
        return {
            ...
            'message': f'Ensemble classified as {best_class} with {confidence:.3f} confidence'
        }
...
async def model_status():
    ...
    global trained_models, transfer_model, ensemble_mode
    
    try:
        return {
            "anfis_loaded": len(trained_models) > 0,
            "transfer_learning_loaded": transfer_model is not None,
            "ensemble_available": ensemble_mode and len(trained_models) > 0 and transfer_model is not None,
        ...
        return {
            "anfis_loaded": False,
            "transfer_learning_loaded": False,
...
async def select_object(
    ...
    from backend.app.utils.color_grading import detect_bell_pepper_colors_opencv, get_average_hsv
    from backend.app.utils.object_detection import (
    ...
    try:
        ...
        # Set default ANFIS result since defect detection is disabled
        anfis_result = {
            'defect_type': 'analysis_disabled',
            'confidence': 0.0,
            'probability': 0.5,
            'features': {},
            'message': 'Defect detection temporarily disabled for accuracy improvements'
        }
        ...
        analysis_summary = generate_analysis_summary(features_dict)
        ...
        response = {
            ...
            "recommendation": "This bell pepper shows quality with color-based ripeness grading.",
            "anfis_defect_detection": anfis_result
        }
    except Exception as e:
